<?php
/*
 * True Typography Helper
 * 
 * @package     True Typography
 * @version     1.0.0 beta
 * 
 * @author      Giant Leap Lab <hello@giantleaplab.com>.
 ...
 * http://kingdesk.com from your website would be appreciated.
 * 
...
class xaFrameworkHelper {
	...
	function __construct($setDefaults = TRUE) {
		...
		// GLL
		$this->chr["leftArrow"] = $this->uchr(8592);
		$this->chr["rightArrow"] = $this->uchr(8594);
		$this->chr["leftRightArrow"] = $this->uchr(8596);
		$this->chr["leftRightArrow"] = $this->uchr(8596);
		$this->chr["leftCeiling"] = $this->uchr(8968);
		$this->chr["rightFloor"] = $this->uchr(8971);
	...
	function set_defaults() {
		...
		// GLL:
		$this->set_smart_extra_symbols();
		$this->set_smart_numbers(FALSE);
		$this->set_smart_numbers_language();
	...
	function set_smart_quotes_language($lang = "en") {
		...
		if(in_array($lang, array('be'/*Belarusian*/, 'ce'/*Catalan*/, 'es'/*Spanish*/, 'lv'/*Latvian*/, 'nb'/*Norsk bokmål (Norway)*/, 'ru'/*Russian*/, 'uk'/*Ukrainian*/))) {
		...
		} elseif(in_array($lang, array('bg'/*Bulgarian*/,'cs'/*Czech*/, 'de'/*German*/, 'et'/*Estonian*/, 'ge'/*Georgian*/, 'hu'/*Hungarian*/, 'lt'/*Lithuanian*/, 'mk'/*Macedonian*/, 'pl'/*Polish*/, 'pt'/*Português*/,'ro'/*Romanian*/, 'sk'/*Slovak*/, 'sr'/*Serbian*/, 'sv'/*Svenska*/))) {
		...
		} elseif(in_array($lang, array('fr'/*French*/, 'eu'/*Euskara (Basque)*/, 'it'/*Italian*/, 'tr'/*Turkish*/))) {	
		...
		} elseif(in_array($lang, array('da'/*Danish*/))) {
	...
	function set_style_caps($on = TRUE) {
		...
		// GLL: Check if PCRE is compiled with "--enable-unicode-properties" and disable style_caps if not
		if($on && @preg_replace("/\p{Lu}+/", "TEST", "TSET") !== 'TEST') $on = FALSE;
	...
	function set_hyphenation_language($lang = "en-US") {
		...
		if (isset($this->settings["hyphenLanguage"]) &&	$this->settings["hyphenLanguage"] == $lang) return TRUE;
		...
		if(file_exists(dirname(__FILE__).'/lang/'.$this->settings["hyphenLanguage"].'.php')) {
			...
			$lang = explode('-', $lang);
			$this->settings["hyphenLanguage"] = array_shift($lang);
	...
	// GLL: Allow inseting some extra symbols
	function set_smart_extra_symbols($on = TRUE){
		
		$this->settings["smartExtraSymbols"] = $on;
		return TRUE;
	}
	
	// GLL: Formats numbers, e.g. 100000.00 -> 100,000.00
	function set_smart_numbers($on = TRUE){
		
		$this->settings["smartNumbers"] = $on;
		return TRUE;
	}
	
	// GLL: Language (format) for numbers, e.g. 100000.00 -> 100,000.00 or 100 000,00
	function set_smart_numbers_language($lang = "en"){
		
		if(in_array($lang, array('be', 'ru', 'ua'))){
		
			$this->settings["decimalPoint"] = ',';
			$this->settings["thousandsSeparator"] = $this->uchr(8201);
	...
	// GLL: Sets lists of patterns and replacements for the replace_custom_patterns function
	function set_patterns_and_replacements($patterns, $replacements){
		
		if(!is_array($patterns) || !is_array($replacements)) return FALSE;
		
		$this->settings['replaceCustomPatterns']['patterns'] = $patterns;
		$this->settings['replaceCustomPatterns']['replacements'] = $replacements;
		
		// Fill out empty replacements
		if(count($patterns) > count($replacements)){
			
			$this->settings['replaceCustomPatterns']['replacements'] = array_merge($this->settings['replaceCustomPatterns']['replacements'], array_fill(count($replacements), count($patterns) - 1, ''));
		}
		
		// Add copies of the initial items but with html entities
		foreach($patterns as $key => $pattern){
			
			if($pattern != htmlentities($pattern)) $this->settings['replaceCustomPatterns']['patterns'][] = htmlentities($pattern);
			if(isset($replacements[$key])) $this->settings['replaceCustomPatterns']['replacements'][] = $replacements[$key];
		}
	...
	function process($html, $isTitle = FALSE) {
		...
		foreach($unlockedTexts as &$unlockedText) {
			...
			$unlockedTextTemp = $unlockedText;
			$unlockedText = $this->smart_math($unlockedTextTemp);
			// GLL: format numbers if the text is not part of a formula
			if($unlockedText['value'] == $unlockedTextTemp['value']) $unlockedText = $this->smart_numbers($unlockedText);
			...
			// GLL
			$unlockedText = $this->replace_custom_patterns($unlockedText);
			$unlockedText = $this->smart_extra_symbols($unlockedText);
...
//		$widow[5] = preg_replace("/\s+/", $this->chr["noBreakSpace"], $widow[5]);
	...
	// GLL: Processes some additional symbols
	function smart_extra_symbols($parsedHTMLtoken){
		
		if(!isset($this->settings["smartExtraSymbols"]) || !$this->settings["smartExtraSymbols"]) return $parsedHTMLtoken;
		$parsedHTMLtoken["value"] = str_replace(array("<->", "&lt;-&gt;"), $this->chr["leftRightArrow"], $parsedHTMLtoken["value"]);
		$parsedHTMLtoken["value"] = str_replace(array("<-", "&lt;-"), $this->chr["leftArrow"], $parsedHTMLtoken["value"]);
		$parsedHTMLtoken["value"] = str_replace(array("->", "-&gt;"), $this->chr["rightArrow"], $parsedHTMLtoken["value"]);
		return $parsedHTMLtoken;		
	}
	
	// GLL: Processes some additional symbols
	function smart_numbers($parsedHTMLtoken){
		
		if(!isset($this->settings["smartNumbers"]) || !$this->settings["smartNumbers"]) return $parsedHTMLtoken;
		$parsedHTMLtoken["value"] = preg_replace_callback(
										
										'/\d+/',
										create_function(
											'$matches',
											'return number_format($matches[0], 2, \'' . $this->settings["decimalPoint"] . '\', \'' . $this->settings["thousandsSeparator"] . '\');'
										),
										$parsedHTMLtoken["value"]
									);
	...
	// GLL: Replace custom patterns
	function replace_custom_patterns($parsedTextToken){
		
		if(!isset($this->settings['replaceCustomPatterns']['patterns']) || !is_array($this->settings['replaceCustomPatterns']['patterns'])
		   || !isset($this->settings['replaceCustomPatterns']['replacements']) || !is_array($this->settings['replaceCustomPatterns']['replacements']))
			  return $parsedTextToken;
		
		$parsedTextToken['value'] = str_replace($this->settings['replaceCustomPatterns']['patterns'], $this->settings['replaceCustomPatterns']['replacements'], $parsedTextToken['value']);
