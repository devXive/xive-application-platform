...
class plgSystemBootstrap extends JPlugin
{
    ...
    public function onBeforeRender()
    {
        ...
        if ($loadCSS) {
            $doc->addStyleSheet(JURI::root(true) . '/media/bootstrap/css/bootstrap.css');
            $doc->addStyleSheet(JURI::root(true) . '/media/bootstrap/css/bootstrap-responsive.css');
        }
        $loadFontAwesome = $this->params->get("loadFontAwesome", TRUE);
        if ($loadFontAwesome) {
            $doc->addStyleSheet(JURI::root(true) . '/media/bootstrap/css/font-awesome.css');
            $doc->addStyleSheet(JURI::root(true) . '/media/bootstrap/css/bootstrap-responsive.css');
        }
        $loadExtendedCSS = $this->params->get("loadExtendedCSS", TRUE);
        if ($loadExtendedCSS) {
            $doc->addStyleSheet(JURI::root(true) . '/media/bootstrap/css/override.css');
            $doc->addStyleSheet(JURI::root(true) . '/media/bootstrap/css/custom.css');
        }
        $loadGoogleCodePrettify = $this->params->get("loadGoogleCodePrettify", TRUE);
        if ($loadGoogleCodePrettify) {
			$doc->addScript(JURI::root(true) . '/media/bootstrap/js/google-code-prettify/prettify.js');
			$doc->addStyleSheet(JURI::root(true) . '/media/bootstrap/js/google-code-prettify/prettify.css');
        }
        ...
        if ($loadJS) {
            ...
            if ($loadJQuery) {
                ...
                if ($jQueryFromLocal) {
                    $doc->addScript(JURI::root(true) . '/media/bootstrap/js/jquery-1.7.1.min.js');
                }else{
